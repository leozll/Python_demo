...
def query_to_workbook(ws,sql_cursor,row_start,data_length):
    style9 = xlwt.XFStyle()
    borders2 = xlwt.Borders()
    borders2.left = xlwt.Borders.MEDIUM
...
def mail_infos():
    ...
    for info in sqlite_query(sqlite3_file,mail_info_sql1):
        ...
        for mail_info in oracle_query(con_string1,mail_info_sql2 % info[1])[0]:
            ...
            info_list.append(mail_info)
        #for mail_info in oracle_query(con_string1,info[1])[0]:
        #    info_tuple = (mail_info[0], mail_info[1], mail_info[2], mail_info[3], mail_info[4], mail_info[5])
        #    info_list.append(mail_info)
    return info_list
...
def send_mail(mail_host,mail_port,mail_user,to_list, sub, content,attachment):
    ...
    for att in attachment:
        xlsxpart = MIMEApplication(open(script_path+'files/'+att+run_time+'.xls', 'rb').read())
        xlsxpart.add_header('Content-Disposition', 'attachment', filename=('utf-8', '', att+'.xls'))
        msg.attach(xlsxpart)
    try:
        server = smtplib.SMTP()
...
def main():
    ...
    for mail_info in mail_infos():
        ...
        if is_sendmail(mail_info)==1:
            ...
            send_key=(mail_info[3],mail_info[4],mail_info[5])
            if send_list.has_key(send_key):
                #send_list[send_key]=send_list[send_key]+';'+mail_info[8]
                if mail_info[6] in send_list[send_key][0]:
                    send_list[send_key] = (send_list[send_key][0], send_list[send_key][1] + ';' + mail_info[8])
                else:
                    send_list[send_key] = ([send_list[send_key][0],mail_info[6]],send_list[send_key][1] + ';' + mail_info[8])
            else:
                send_list[send_key] = ([mail_info[6]], mail_info[8])
        else:
    ...
    for mail_info in mail_info_list:
        ...
        file_name=mail_info[6]+run_time
        att_name=mail_info[6]
        sub=mail_info[5].decode('utf-8')
        report_info_list={}
        print '5:',data_length
        if os.path.exists(script_path + 'files/' + file_name + '.xls'):
            w=xlrd.open_workbook(script_path + 'files/' + file_name + '.xls',formatting_info=True)
            wb=copy(w)
            for l in range(0,len(w.sheets())):
                print data_length_dict
                print report_name
                print w.sheet_by_index(l).name
                sheet_name=w.sheet_by_index(l).name
                print type(sheet_name)
                print data_length_dict[sheet_name]
                data_length=data_length_dict[sheet_name]
                print '4:',data_length
                ws1 = wb.get_sheet(l)
                ws1.insert_bitmap('./bea.bmp', 1, 1)
                # global data_length
                ws1.insert_bitmap('./ctrip.bmp', 1, data_length + 3)
        else:
            wb = xlwt.Workbook()
        ws = wb.add_sheet(mail_info[1].decode('utf-8'), cell_overwrite_ok=True)
        ...
        for chart_seq,report_infos in enumerate(sorted(report_info_list.items(),key=lambda item:item[1])):
            ...
            if chart_seq==0:
                data_length=query_to_workbook_title(ws,oracle_query(con_string3, report_data_sql),row_start)
                if not data_length_dict.has_key(send_key):
                    data_length_dict[report_name.decode('utf-8')]=data_length
                print 'first:',data_length
            #将可视化写入excel文件
            query_to_workbook(ws,oracle_query(con_string3, report_data_sql),row_start,data_length)
            #从写入的行数开始继续写入
        ...
        # 保存excel文件
        wb.save(script_path + 'files/' + file_name + '.xls')
        print 'sec:',data_length



       #send_mail(mail_host, mail_user, mail_pass, mail_info[5], att_name, content,file_name)
        #send_mail(server_infos_list[0], server_infos_list[1], server_infos_list[2], mail_info[5], sub, content, file_name,att_name)
    for send_k,send_v in send_list.items():
        print send_list
        send_mail(server_infos_list[0], server_infos_list[1], server_infos_list[2], send_v[1], send_k[2].decode('utf-8'), content, send_v[0])
#send_mail(mail_host,mail_port,mail_user,to_list, sub, content,attachment,att_name):
