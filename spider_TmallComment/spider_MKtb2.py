...
if __name__ == '__main__':
	...
	'''
	# 打开指定页面
	url= 'https://s.taobao.com/search?q=%E8%88%92%E6%B4%BB%E7%9C%BC%E8%86%9C%E9%9C%9C&imgfile=&commend=all&ssid=s5-e&search_type=item&sourceId=tb.index&spm=a21bo.50862.201856-taobao-item.1&ie=utf8&initiative_id=tbindexz_20170120'
	...
	row0 = [u'宝贝标题', u'产品名称', u'店铺名称', u'商品id', u'淘宝价', u'交易成功个数', u'收藏个数', 'url', u'原价', u'分类', u'品牌', u'功效', u'售出数量']
	# 生成第一行
	...
	for n,i in enumerate(f.readlines()):
		...
		try:
			...
			infos = html.xpath("//ul[@class='attributes-list']/li")
			category = ''
			brand = ''
			function = ''
			for info in infos:
				if info.text.find(u'分类')>-1:
					category = info.text
				if info.text.find(u'品牌')>-1:
					brand = info.text
				if info.text.find(u'功效')>-1:
					function = info.text
			...
			originalprice = jasonData[u'data'][u'price']
			...
			sheet1.write(n + 1, 7, i)
			sheet1.write(n + 1, 8, originalprice)
			sheet1.write(n + 1, 9, category)
			sheet1.write(n + 1, 10, brand)
			sheet1.write(n + 1, 11, function)
			sheet1.write(n + 1, 12, soldTotalCount)
		except Exception, e:
			print i,Exception,e
			f2.write(i+str(Exception)+str(e) + '\n')
	f.close()
	f2.close()
	excelfile.save('mktb2.xls')
