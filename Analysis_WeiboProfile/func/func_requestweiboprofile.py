...
@retry(retry_on_result=retry_if_result_none, wait_fixed=1000, stop_max_attempt_number=5)

def weibo_profile(p_kw):
    logger.info(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + brnd_kw + '解析中...')
