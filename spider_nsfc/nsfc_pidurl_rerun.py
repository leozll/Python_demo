...
def exceptionPages():
	...
	cur=conn.cursor()
	cur.execute("DELETE FROM nsfc.dbo.nsfcLinkInfo WHERE PAGE IN (SELECT page FROM (SELECT PAGE,COUNT(1) c FROM nsfc.dbo.nsfcLinkInfo GROUP BY PAGE) a WHERE a.c<>7)")
	conn.commit()
	
	cur.execute("select distinct page from nsfc.dbo.nsfcLinkInfo")
...
if __name__ == '__main__':
	...
	#driver.find_element_by_id('currentPageGo').send_keys('123')
	#driver.find_element_by_class_name('go').find_element_by_tag_name('a').click()
	#print driver.find_elements_by_id('currentPageNav')[0].text
	#driver.find_elements_by_class_name('page')[0].find_element_by_link_text(u'下一页').click()
	#print driver.find_elements_by_id('currentPageNav')[0].text
	#print driver.find_elements_by_class_name('time_dl')[0].find_element_by_tag_name('dt').find_element_by_tag_name('a').get_attribute("href")
	
	resList=exceptionPages()
	Pages=[]
	for i in resList:
		Pages.append(int(i[0]))
