...
def handler(event, context):
    ...
    if get_col(ots_client, tbl, pk, ['update_time'], None) == 'NoData' or get_col(ots_client, tbl, pk, ['status'],
                                                                                  None) != ','.join(uids):
    ...
    print(str(checkpoint) + ' ' + str(len(uids)) + ' ' + str(get_col(ots_client, tbl, pk, ['name'], None)))
    # if all the uids are analysed today
    if checkpoint + 1 >= len(uids) and get_col(ots_client, tbl, pk, ['name'], None) == datekey_uid:
        logger.info((datetime.datetime.utcnow() + datetime.timedelta(hours=8)).strftime(
            '%Y-%m-%d %H:%M:%S') + ' Step 5 :  all the uids are analysed today! exit the script! (checkpoint:' + str(
            checkpoint) + ' totally:' + str(len(uids)) + ')')
    ...
    for i in range(checkpoint, endp):
        logger.info((datetime.datetime.utcnow() + datetime.timedelta(hours=8)).strftime(
            '%Y-%m-%d %H:%M:%S') + ' Step 5 (' + str(i + 1) + '/' + str(endp) + ') : start to analyse uid: ' + uids[i])
        request_status = ''
        # loop the months
        for d in d_list:
            try:
                ...
                logger.info((datetime.datetime.utcnow() + datetime.timedelta(hours=8)).strftime(
                    '%Y-%m-%d %H:%M:%S') + '"' + str(uids[i]) + ' ' + str(d) + ' "解析失败????...：' + str(e))
            time.sleep(1)
        # if finished
        pk = [('uid', int(uids[i]))]
    ...
    if checkpoint + 1 >= len(uids) and get_col(ots_client, tbl, pk, ['name'], None) == datekey_uid:
        logger.info((datetime.datetime.utcnow() + datetime.timedelta(hours=8)).strftime(
            '%Y-%m-%d %H:%M:%S') + ' Step 6 :  all the uids are analysed today! exit the script! (checkpoint:' + str(
            checkpoint) + ' totally:' + str(len(uids)) + ')')
        sub = str(len(uids)) + ' uids are requested finished! pls check the log?'
        content = 'log link : XXXXX?'
        send_mail(user_list, sub, content)
    return 'Congratulations! ' + (datetime.datetime.utcnow() + datetime.timedelta(hours=8)).strftime(
        '%Y-%m-%d %H:%M:%S')
